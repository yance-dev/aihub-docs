...
| 工作流模块  | 工作流模块是从具体工作流节点中抽象出的可复用工作单元，其设计理念类似于“可重入函数”，具备如下核心特性：1. 单一职责：一个模块完成一个清晰、明确的任务。名称准确表达其功能。  2. 通用性：具备通用逻辑、良好参数设计，能在多个场景复用   3.确定性：模块的输出仅由其输入决定，不依赖外部不可见状态（如硬编码路径、环境变量），输入（输入参数，环境变量）相同时应产生相同输出；模块开发文档请参考：[工作流模块开发指引](#workflow-guide)|
| 工作流模板    | 工作流模板是将经过验证、调试的工作流版本进行固化和抽象的产物。它作为一种可复用的蓝图，允许用户基于此快速创建新的工作流，从而避免重复构建，并促进流程标准化。 |
| 运行实例   | 运行实例是工作流的单次执行。|
...
### 调试模块
调试模块与调试工作流节点类似，区别在于配置参数不同，模块调试需要自行配置节点类型，sku数量，环境变量和存储挂载。详细调试方法请参考[调试工作流节点](#debugging-workflow-nodes)

## 模板管理
在模板管理可以查看所有已发布和未发布的工作流模板，并支持创建工作流模板。
![](images/工作流管理/image44.png)

### 创建模板

点击“创建工作流模板”按钮，跳转创建工作流模板页面，依次填写相关参数后，点击“创建模板”。
![](images/工作流管理/image45.png)

参数说明：

1. 模板名称：请填写清晰表明模板作用的名称，支持小写字母数字和-。
2. 工作流选择：依次选择要转为模板的工作流和具体版本。
3. 标签：选择相应的模板标签。
4. 输入参数：无需填写，选择工作流和版本后，将自动展示对应的初始输入参数。
5. 输出参数：请选择该工作流版本特定节点（源节点名称）的特定输出参数（源节点输出参数）作为整个工作流模板的输出参数，并定义一下模板输出参数的参数名（输出参数）。
6. 描述：请简要描述模板功能。

## 模块/模板审核

用户可在模块/模板审核页面查看工作流模块和模板的审核情况，可以审核通过或拒绝用户审核请求，审核不通过时，需说明不通过的理由；审核通过后，模块变为已发布状态，全体用户可在工作流中自由引用使用。

由各组leader统筹本组模块/模板审核，可leader自行审核或安排同组同事交叉审核，审核人需对提交的模块/模板代码进行审核把关，确保模块/模板：
...
工作流模板的状态流转与模块相似，都需要经过审核才可向所有用户开放，与模块不同的是：

1. 工作流模板创建后默认为构建成功状态，没有构建中状态；
2. 如果模板审核不通过，不能编辑后重新提交审核，需重新创建新的工作流模板进行发布和审核；
...
4. **初始输入参数**：初始输入参数为工作流提供初始数据输入，在创建运行实例时赋值。输入参数类型：

    * `文本`：普通文本类型；

    * `数据集`： AIHub数据集管理的数据集资源， 实际传给工作流节点是数据集ID，需要节点自行解析访问资源。

    * `模型`： AIHub模型管理的模型资源， 实际传给工作流节点是模型ID，需要节点自行解析访问资源。

    数据集和模型解析示例代码：
    ```
    from aihub.client import Client
    # 假设该工作流初始输入参数的数据集参数名称为dataset_version_id，模型参数名称为model_id

    # 初始化client
    client = Client(base_url=your_base_url, token=TOKEN)

    # 获取数据集路径
    dataset = client.dataset_management.get_dataset_version($dataset_version_id)
    print(dataset.file_storage_path) # 数据集路径。访问该路径，请注意在节点挂载数据存储aihub_dataset

    # 获取模型路径
    model_db = client.model_center.get_model_db(id=$model_id)
    print(model_db.file_storage_path)# 模型路径。访问该路径，请注意在节点挂载数据存储aihub_modelcenter
    ```

![](images/工作流管理/diagram-8.jpg)
...
3. **镜像地址**：内网请填写 99.199 镜像仓库 的镜像地址，外网请填写 14.129 镜像仓库 的镜像地址。
...
在创建工作流过程中，可随时将正在创建的工作流暂存为草稿或执行试运行。点击暂存后，数据被存储到工作流，下次编辑时可在【工作流 - 草稿箱】找到，可继续编辑或删除草稿。

![](images/工作流管理/image-49.png)

![](images/工作流管理/image-50.png)

### 试运行

用户在编辑工作流时，可随时点击右上角的【试运行】按钮，打开试运行配置页面，填写相关参数后，点击【开始试运行】，即可在不保存工作流的情况下，执行试运行，查看运行结果。

![](images/工作流管理/image-51.png)

填写试运行参数：

![](images/工作流管理/image-52.png)

查看试运行状态：

![](images/工作流管理/image-53.png)

![](images/工作流管理/image-54.png)
...
### 调试工作流节点{#debugging-workflow-nodes}
在工作流详情 - 节点详情 点击【创建调试】按钮，填写相关信息，即可一键创建调试任务。

调试任务将自动使用节点镜像、SKU 配置、环境变量和存储挂载，并生成独立的调试容器。

所有调试任务运行于独立的 4090 Debug 集群，无需排队。

![](images/工作流管理/image-39.png)

等待镜像拉取完成后，点击【打开调试】按钮，将自动打开一个调试终端，可以直接在终端输入命令进行调试。

![](images/工作流管理/image-40.png)
![](images/工作流管理/image-41.png)
![](images/工作流管理/image-42.png)

调试任务可手动关闭，或在达到超时时间后自动终止。

![](images/工作流管理/image-43.png)
## 运行实例
...
模板节点添加引用与模块节点类似，可正常接收上游节点参数和向下游节点传递参数。需要特别注意的点：

1. 模板节点的输入参数只能是普通文本类型，不支持文件路径类型；
2. 编辑模式下工作流模板不能展开；

![](images/工作流管理/image46.png)

在运行实例中，模板节点默认是收起状态，可以点击右上角的加号展开模板节点，查看节点详情。

![](images/工作流管理/image47.png)

![](images/工作流管理/image48.png)
